...
MatrixKeyboard :: MatrixKeyboard(Port* R0_Port, Port* R1_Port, Port* R2_Port, Port* R3_Port,
...
uint8_t MatrixKeyboard :: SearchPressedButton(uint8_t row)
{
	uint8_t column;	
...
bool MatrixKeyboard :: ButtonIsPressed()
{
	for(uint8_t row = 0; row < RowsNum; row++)
	{
		...
		tempPressedButton = SearchPressedButton(row);
